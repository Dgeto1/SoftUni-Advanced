    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while ((commands = Console.ReadLine()) != "END")
            {
                ...
                if (segments[0] != "swap" || int.Parse(segments[1]) >= matrix.GetLength(0) || int.Parse(segments[2]) >= matrix.GetLength(0) || int.Parse(segments[3]) >= matrix.GetLength(1) || int.Parse(segments[4]) >= matrix.GetLength(1))
                {
